...
@router.get("")
def list_roles(ctx: AuthContext = Depends(require_perm("user:role_list")),
...
def create_role(req: RoleCreate, request: Request,
                ctx: AuthContext = Depends(require_perm("user:role_create")),
                db: Session = Depends(get_db)):
...
def role_detail(role_id: int,
                ctx: AuthContext = Depends(require_perm("user:role_list")),
                db: Session = Depends(get_db)):
    """角色详情：权限码列表 + 绑定用户。"""
...
def update_role(role_id: int, req: RoleUpdate, request: Request,
                ctx: AuthContext = Depends(require_perm("user:role_update")),
                db: Session = Depends(get_db)):
...
def delete_role(role_id: int, request: Request,
                ctx: AuthContext = Depends(require_perm("user:role_delete")),
                db: Session = Depends(get_db)):
    """删除（仅非内置；拦截：仍有用户绑定）。"""
...
def assign_role_perms(role_id: int, req: RolePermAssignReq, request: Request,
                      ctx: AuthContext = Depends(require_perm("user:role_assign")),
                      db: Session = Depends(get_db)):
