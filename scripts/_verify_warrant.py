...
print(f"warrant_service  主表函数覆盖: {len(main) - len(missing_m)}/{len(main)}")
if missing_m: print(f"  ❌ missing: {missing_m}")
print(f"ext_service      re-export 覆盖: {len(ext) - len(missing_e)}/{len(ext)}")
